...
# FORCE: must be set in .env, no fallback default
CDP_URL = os.environ["CDP_URL"]  # e.g., "http://127.0.0.1:9223"

# Flow pacing / polling knobs
...
# Extra settle and inter-message pacing for legacy modes (kept for completeness)
FIRST_MESSAGE_GRACE_S = float(os.getenv("FIRST_MESSAGE_GRACE_S", "15.0"))
...
# --- Pacing/breaks & safety cooldown ---
MINI_BREAK_EVERY_N = int(os.getenv("MINI_BREAK_EVERY_N", "15"))
MINI_BREAK_MIN_S   = int(os.getenv("MINI_BREAK_MIN_S", str(8 * 60)))
MINI_BREAK_MAX_S   = int(os.getenv("MINI_BREAK_MAX_S", str(14 * 60)))

LONG_BREAK_EVERY_N = int(os.getenv("LONG_BREAK_EVERY_N", "100"))
LONG_BREAK_MIN_S   = int(os.getenv("LONG_BREAK_MIN_S", str(30 * 60)))
LONG_BREAK_MAX_S   = int(os.getenv("LONG_BREAK_MAX_S", str(48 * 60)))

MAX_CONTINUOUS_RUNTIME_S = int(os.getenv("MAX_CONTINUOUS_RUNTIME_S", str(12 * 60 * 60)))
COOLDOWN_RUNTIME_S       = int(os.getenv("COOLDOWN_RUNTIME_S",       str(6 * 60 * 60)))

# --- Instance-specific run settings (centralized here; still set in .env) ---
